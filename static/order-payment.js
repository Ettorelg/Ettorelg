window.AlphaPayment = (() => {
  ...
  async function open(order, {csrf, onComplete=()=>{}, initialPayment='contanti', initialTender='', autoSubmit=false}={}) {
    ...
    try {
      ...
      if(info.pagamento&&!['emesso','registrato'].includes(info.pagamento.stato)&&info.can_resolve&&info.config?.brand==='axon_micrelec'){const emitted=element(tools,'button','✓ Già emesso dal DADO');const proof=element(dialog,'section',undefined,'payment-proof');proof.hidden=true;const zLabel=element(proof,'label','Chiusura Z DADO');const zno=element(zLabel,'input');zno.type='number';zno.min='1';const dLabel=element(proof,'label','Numero documento DADO');const documentNo=element(dLabel,'input');documentNo.type='number';documentNo.min='1';const nLabel=element(proof,'label','Verifica effettuata');const note=element(nLabel,'input');note.maxLength=500;note.placeholder='Es. verificato scontrino cartaceo';const confirmEmitted=element(proof,'button','Conferma documento già emesso');confirmEmitted.type='button';emitted.onclick=()=>{proof.hidden=!proof.hidden;if(!proof.hidden)zno.focus()};confirmEmitted.onclick=async()=>{if(!zno.value||!documentNo.value||note.value.trim().length<12){setMessage('Inserisci chiusura, documento e una nota di verifica.',true);return}setBusy(true);try{await api(paymentUrl+'/conferma-emesso',{confirmation:'CONFERMO EMESSO',zno:zno.value,document:documentNo.value,note:note.value.trim()},csrf);await onComplete();dialog.close()}catch(error){setMessage(error.message,true)}finally{setBusy(false)}}}

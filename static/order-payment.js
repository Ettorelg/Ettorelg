window.AlphaPayment = (() => {
  ...
  async function open(order, {csrf, onComplete=()=>{}, initialPayment='contanti', initialTender=''}={}) {
    ...
    try {
      ...
      if(info.pagamento&&!['emesso','registrato'].includes(info.pagamento.stato)&&info.can_resolve&&info.config?.brand==='axon_micrelec'){const emitted=element(tools,'button','✓ Già emesso dal DADO');emitted.onclick=async()=>{const confirmation=prompt('Usa questa funzione solo se lo scontrino è realmente uscito dal DADO. Scrivi CONFERMO EMESSO');if(confirmation!=='CONFERMO EMESSO')return;const zno=prompt('Numero chiusura Z del documento DADO:');const documentNo=prompt('Numero documento DADO:');const note=prompt('Descrivi la verifica effettuata (minimo 12 caratteri):');if(!note)return;setBusy(true);try{await api(paymentUrl+'/conferma-emesso',{confirmation,zno,document:documentNo,note},csrf);await onComplete();dialog.close()}catch(error){setMessage(error.message,true)}finally{setBusy(false)}}}
